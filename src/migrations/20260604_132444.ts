...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "public"."enum_posts_status" AS ENUM('pending', 'published', 'rejected');
  ...
  CREATE TABLE "users" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"email" varchar NOT NULL,
  	"display_name" varchar DEFAULT 'Anonyme courageux' NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  
  CREATE TABLE "posts_red_flags" (
  	"_order" integer NOT NULL,
  	"_parent_id" integer NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  ...
  CREATE TABLE "posts" (
  	...
  	"author_name" varchar DEFAULT 'Anonyme' NOT NULL,
  	"user_id" integer,
  	"status" "enum_posts_status" DEFAULT 'pending' NOT NULL,
  	"budget" varchar,
  	"deadline" varchar,
  	"cover_id" integer,
  ...
  ALTER TABLE "posts_red_flags" ADD CONSTRAINT "posts_red_flags_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."posts"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE INDEX "posts_red_flags_order_idx" ON "posts_red_flags" USING btree ("_order");
  CREATE INDEX "posts_red_flags_parent_id_idx" ON "posts_red_flags" USING btree ("_parent_id");
  ...
  CREATE UNIQUE INDEX "magic_links_token_hash_idx" ON "magic_links" USING btree ("token_hash");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "posts_red_flags" CASCADE;
